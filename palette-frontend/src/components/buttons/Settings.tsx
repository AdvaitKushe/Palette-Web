...
export const Settings = ({ className, ...props }: ComponentProps<'div'>) => {
  ...
  const initSettings = async () => {
    ...
    // try {
    //   await window.api.saveSettings(settings).then((response) => {
  ...
  return (
    <div
      ref={dropdownRef}
      className={twMerge('flex flex-row items-center justify-center z-50 align-middle', isOpen ? 'bg-zinc-600/40  text-white rounded-md' : '', className)}
      {...props}
    >
      <ActionButton
        onClick={() => {
